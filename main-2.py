...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # WD MANUAL STEP 1
    # =========================
    if user_id in WD_STEP and isinstance(WD_STEP[user_id], dict) and WD_STEP[user_id].get("step") == "manual_amount":
        try:
            jumlah = int(text)
        except:
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = query.data
    # FIX: Definisikan parts di sini supaya tersedia di semua blok if/elif di bawah
    parts = data.split("_")

    # WD NOMINAL
    if data.startswith("wd_nominal_"):
        ...
        if jumlah < MIN_WD:
            await query.message.reply_text(f"❌ Minimal WD Rp {MIN_WD:,}")
            return
        if jumlah > saldo:
            await query.message.reply_text(f"❌ Saldo tidak cukup! Saldo kamu: Rp {saldo:,}")
            return

        WD_STEP[user_id] = {"step": "input_nomor", "jumlah": jumlah}
        context.user_data["wd_jumlah"] = jumlah
        await query.message.reply_text("📱 Masukkan nomor DANA / Bank tujuan:")
        return

    # WD MANUAL
    if data == "wd_manual":
        user_id = query.from_user.id
        # FIX: Gunakan dict agar konsisten dengan pengecekan di text_handler
        WD_STEP[user_id] = {"step": "manual_amount"}
        ...
        if await is_member(context.bot, user_id):
            await query.edit_message_text("✅ Verifikasi berhasil! Kamu sudah join channel.\nSilakan gunakan bot kembali.")
        else:
            await query.answer("⚠️ Kamu belum join channel!", show_alert=True)
        return
        ...
        update_saldo(user_id, jumlah)
        try:
            await context.bot.send_message(user_id, f"❌ *WD DITOLAK!*\nRp {jumlah:,} dikembalikan ke saldo.", parse_mode="Markdown")
        except: pass
        await query.edit_message_text(query.message.text + "\n\n❌ DITOLAK & DIKEMBALIKAN", parse_mode="Markdown")
        ...
        update_saldo(user_id, jumlah)
        try:
            await context.bot.send_message(user_id, f"✅ *Deposit DISETUJUI!*\nRp {jumlah:,} masuk ke saldo!", parse_mode="Markdown")
        except: pass
        await query.edit_message_caption(caption=query.message.caption + "\n\n✅ DEPOSIT APPROVED", parse_mode="Markdown")
        ...
        user_id = query.from_user.id
        wds = get_user_wd_history(user_id)
        if not wds:
            await query.edit_message_text("💸 Belum ada riwayat WD.")
            return
        text = "💸 *RIWAYAT WD*\n━━━━━━━━━━━━━━━━━━\n\n"
        for w in wds[:10]:
            status_icon = "✅" if w[3] == "approved" else ("❌" if w[3] == "rejected" else "⏳")
            text += f"{status_icon} Rp {w[2]:,} → {w[4]}\n"
        await query.edit_message_text(text, parse_mode="Markdown")
...
def main():
    ...
    # Message handlers
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex("^📋 Tugas$"), show_tugas))
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex("^💰 Saldo$"), show_saldo))
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex("^💸 Tarik Saldo$"), show_wd))
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex("^👥 Referral$"), show_referral))
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex("^📜 Riwayat$"), show_riwayat_menu))
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex("^ℹ️ Info$"), show_info))
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex("^💳 Deposit$"), show_deposit))
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex("^🤖 Bot Lainnya$"), show_bot_lainnya))

    # General text handler (untuk WD manual step, broadcast, dll)
